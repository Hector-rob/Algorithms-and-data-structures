...
Dato::Dato(string smes, int dia, int hora, int min, int seg, string ip, string razon) {
    ...
    string resrazon;
    if (razon.find("Illegal user") != string::npos) resrazon = "iu"; //comprime los datos en un resumen
...
string Dato::decodeResumen(string resumen) { //O(1), Toma el resumen y lo "descomprime" para restaurar los datos originales
    ...
    return dia + "/" + mes + " " + hora + ":" + min + ":" + seg + " " + razon;
}

Dato Dato::leerString(string st) { //O(1), Construye un dato a partir de un String
...
string Dato::toString(Dato d) { //O(1), imprime una representación textual de los detalles del dato
    stringstream s;
